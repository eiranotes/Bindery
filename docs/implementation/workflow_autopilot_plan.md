...
|---|---|---|
| `runProjectStarterAutopilot(userNote?)` | idea-discovery(3) | 없음 (후보 카드만) |
...
| `runEpisodeAutopilot({episode, userNote?, candidateCount?})` | episode-brief → scene-plan → draft-candidate ×N | 없음 — 후보 파일만. 브리프·장면 계획은 autopilot 자동 승인(soft) |
...
- 무입력 `runEpisodeAutopilot` → 기준 누락 시 차단, 준비 후 설계 자동 승인(approvedBy: autopilot) + 라벨 후보 3개 + 원고 불변
