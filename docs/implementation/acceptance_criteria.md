...
- 구조화 ZIP도 가져오기 전에 원본 resume 이어쓰기와 ep001 재시작을 선택하며, 플롯의 최장 20화를
  기본 8화로 줄이지 않고 보존한다.
- 기존 바이블의 ep001 재시작과 resume 이어쓰기 모두 홈에서 500자 추가 지시를 받는다. 지시는
  `[사용자 추가 지시]` 구획으로 플롯(생성 시)·회차 브리프·장면 계획·초안 후보에 전달되며, 기존
  회차 설계가 있어도 지시 입력 시 브리프와 장면 계획을 다시 생성한다.
...
- dev 런타임: 간단/설계자 13화면 모두 진입, 현재 샘플 본문 CTA 86개, 이름 없는 버튼 0,
  콘솔 오류 0.
- 390/768/1280px: 13화면 전체 문서/작업면 수평 overflow 0. 추가 지시 입력이 있는 ep015 이어쓰기
  화면도 세 viewport에서 overflow 0, 입력·이어쓰기 CTA·ep001 재시작 CTA 노출, 콘솔 오류 0.
- 초기 production JS: 883.40kB → 246.18kB. 지연 로드 청크는 모두 205kB 이하, 500kB warning 제거.
...
- 상세 증거: `docs/UI_CTA_PERFORMANCE_REVIEW_20260710.md` 및
  ...
  `.superloopy/evidence/frontend/2026-07-10-start-instruction/`.
...
- `npm test` — 82/82 통과 (코어 단위 + 시나리오 E2E + ep001 재작성 범위/회차 설계 + 재시작·이어쓰기 추가 지시 프롬프트 + 자료 업로드/구조화 zip + 실제 agy 사용량 파싱 + 사용량/문체/백업/품질/외부변경).
...
- `npm run build` — OK. 초기 JS 246.18kB, chunk-size warning 없음.
...
최신 검증: `npm run check` 0 오류 · `npm test` 82/82 · `npm run build` OK · 실제 agy 후보 1개
