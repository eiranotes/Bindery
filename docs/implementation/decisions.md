...
원본 이어쓰기는 그 지점(예: ep015)을 보존하고, 다시 쓰기는 바이블·인물·세계관·플롯을 유지한 채
resume만 snapshot 후 ep001로 재설정한다. 두 동작은 버튼 문구와 실행 함수부터 분리하며, 기존 원고를
자동 삭제하거나 덮어쓰지 않는다. 플롯 회차 수는 기본 8화로 축소하지 않고 원본의 최장 회차를 보존한다.
