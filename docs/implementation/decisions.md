...
### D31. 실제 공급자 쿼터를 우선하고, 없을 때만 원장 기반 비용을 추정한다
일반 CLI는 구조화된 실제 usage를 주지 않으므로 토큰을 글자 수 기반으로 추정한다(`usage.ts`).
러너는 실행마다 `.bindery/usage.json`에 토큰만 append하고, 요금은 표시 시점에 설정의 모델
단가 테이블로 곱한다. 단, `agy`는 대화형 `/usage`가 제공하는 5시간·주간 잔여 한도를 tmux의
격리 세션에서 읽어 `.bindery/provider-usage.json`에 보관한다. agy 선택 중에는 이 실제 쿼터가
홈·상단바·설정의 진실이며 추정 비용과 월 예산 경고는 노출하지 않는다. 다른 실행기와 조회 실패
환경에서만 기존 원장 기반 추정을 유지한다. fallback·오프라인 실행은 항상 $0이다.
...
### D45. 같은 작품의 시작 지점은 원본 이어쓰기와 ep001 재시작으로 분리한다
구조화 ZIP의 resume와 이미 열린 작품의 resume는 다음 작업 지점을 결정하는 source of truth다.
원본 이어쓰기는 그 지점(예: ep015)과 활성 플롯을 보존한다. 다시 쓰기는 바이블·인물·세계 규칙과
가져온 원본 플롯 문서는 유지하되, resume·진행 상태·ep001 원고/브리프/장면 계획/후보 목록과 활성
플롯을 snapshot 후 재시작 상태로 바꾼다. 재시작 플롯은 원본의 과거형 사건도 완료 이력이 아닌
재작성 설계 입력으로 읽고 ep001부터 원본의 최장 회차까지 모든 row를 요구한다. 모델이 다시 ep015
이후만 반환하면 semantic validation 실패로 repair하고, 끝내 범위를 채우지 못하면 ep001부터의 로컬
뼈대를 만들어 집필 파이프라인이 잘못된 회차로 되돌아가지 않게 한다.

### D46. agy 실제 쿼터 조회는 일반 프롬프트 실행 경로와 분리한다
`agy -p '/usage'`는 슬래시 명령을 실행하지 않고 일반 프롬프트로 보내므로 사용하지 않는다.
dev와 Tauri 브리지는 별도 tmux 세션에서 agy 대화형 UI를 실행하고 `/usage`를 입력한 뒤 화면을
캡처한다. 파서는 모델 그룹별 5시간·주간 잔여율과 갱신 시간을 구조화하며, 격리 세션은 성공·실패와
무관하게 종료한다. tmux가 없거나 출력 형식이 달라 파싱할 수 없으면 추정치로 위장하지 않고 조회
실패를 표시한다.

### D47. 시작 추가 지시는 별도 프롬프트 계약으로 강제한다
기존 바이블에서 ep001 재시작 또는 resume 지점 이어쓰기를 선택할 때 최대 500자의 추가 지시를
받는다. 원문은 `intentNote`에 유지하고 실행 직전에 `[사용자 추가 지시]`와 적용 규칙으로 감싸
플롯 생성이 필요한 흐름의 플롯 프롬프트와 회차 브리프·장면 계획·초안 후보 프롬프트에 동일하게
전달한다. 추가 지시가 있으면 이미 저장된 브리프와 장면 계획을 재사용하지 않는다. 바이블과
충돌하는 지시는 바이블을 자동 변경하는 대신 위험으로 명시하도록 프롬프트 계약에 고정한다.

### D48. 사용자 작성 초기 프롬프트는 실행 trace와 분리해 프로젝트에 보존한다
실행기에 전달한 전체 프롬프트는 기존처럼 `.bindery/trace/`에 남기되, 사용자가 기획 시작 때 직접
작성한 최대 500자의 작품 방향은 `.bindery/initial-planning-prompt.md`에 원문으로 저장한다. 이 파일은
프로젝트 백업에 포함되는 source-of-truth 텍스트이며, 기획 후보 확인과 초안 후보 단계에서 같은 값을
편집한다. 편집 중에는 이전 기획안과 초안 후보 적용을 잠그고, 재작성 실행이 저장과 함께 브리프,
장면 계획, 초안 후보를 다시 만든다. 현재 원고에는 자동 반영하지 않는다. agy의 우상단 실제 쿼터는
모델 그룹별 5시간·주간 잔여율 중 각각 최솟값을 `5h`와 `1주` 한 쌍으로 표시한다.
