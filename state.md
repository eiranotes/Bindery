...
## 2026-07-10 초기 기획 프롬프트 보존·초안 재작성·상단 이중 한도

- 초기 기획: 홈에서 사용자가 직접 쓴 최대 500자 프롬프트를
  `.bindery/initial-planning-prompt.md`에 저장한다. 실행 전체 trace와 분리된 프로젝트 원문이며
  백업 대상이다.
- 기획 확인: 기획 후보 아래에 실제 사용한 프롬프트를 다시 보여준다. 수정하면 기존 기획 채택을
  잠그고 [수정한 프롬프트로 다시 만들기]를 거쳐야 한다.
- 초안 재작성: 집필의 초안 후보 단계에서 저장 프롬프트를 편집할 수 있다. 수정본으로 다시 쓰면
  브리프·장면 계획·초안 후보를 재생성하고 현재 원고는 건드리지 않는다. 수정 중에는 이전 후보
  적용을 잠가 입력과 결과의 계보가 섞이지 않게 한다.
- 기획 전달: 기획 후보 채택 시에도 같은 프롬프트가 세계관 확장과 플롯 설계에 이어진다.
- 한도: agy 선택 시 우상단에 `5h`와 `1주` 잔여율을 항상 함께 표시한다. 여러 모델 그룹에서는
  각 창의 가장 낮은 값을 표시하고, 조회 전에는 `--`로 자리를 유지한다.
- 검증: Svelte check 0/0, 프롬프트·autopilot·usage 27/27, 전체 테스트 85/85, CTA 178개
  연결 감사 0 누락, production build 성공. 390/768/1280 실브라우저에서 프롬프트 편집 상태,
  후보 잠금, 이중 한도, 수평 overflow 0을 확인했다.
