...
`epXXX 이어쓰기`와 `같은 설정으로 ep001부터 다시 쓰기`를 별도 동작으로 제공한다. 두 방식 모두
시작 전에 최대 500자의 추가 지시를 받을 수 있다. 입력한 지시는 `[사용자 추가 지시]` 계약으로
플롯 생성이 필요한 경우의 플롯, 회차 브리프, 장면 계획, 초안 후보 프롬프트에 포함되며, 기존
회차 설계가 있어도 지시가 빠지지 않도록 브리프와 장면 계획을 다시 만든다. 사용자가 직접 쓴
초기 프롬프트는 `.bindery/initial-planning-prompt.md`에 별도로 보존한다. 기획안 확인과 초안 후보
단계에서 원문을 다시 보고 수정할 수 있으며, 수정본으로 재작성하면 회차 설계와 초안 후보만 새로
만들고 현재 원고는 유지한다.

실행기가 `agy`이면 설정 화면은 프로젝트 토큰으로 요금을 추정하지 않고, 대화형 `/usage` 화면의
5시간·주간 잔여 한도를 직접 읽어 표시한다. 두 한도는 우상단에 `5h`와 `1주`로 항상 함께 표시되며,
모델 그룹이 여러 개면 가장 낮은 잔여율을 보수적으로 보여준다. 다른 실행기는 실제 쿼터 API가
없을 때만 기존 실행 원장 기반 추정치를 표시한다.
